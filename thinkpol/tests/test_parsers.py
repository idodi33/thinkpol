...
import parsers
#from ..parsers import parsers, parser_utils 
from . import test_utils as tu


@pytest.fixture
def mock_data_dir(monkeypatch, tmp_path):
	monkeypatch.setattr(parsers.parser_utils, 'DATA_DIR', tmp_path)


def assert_user_info(field):
	print(f"assert_user_info: DATA_DIR is {parsers.parser_utils.DATA_DIR}")
	parser = parsers.parsers[field]
	js = parser(tu._JSON)
...
def test_image(mock_data_dir, format):
	js_dict = assert_user_info(format)
	...
	file_path = parsers.parser_utils.DATA_DIR / formatted_time / (format + ".jpg")
...
def test_feelings(mock_data_dir):
